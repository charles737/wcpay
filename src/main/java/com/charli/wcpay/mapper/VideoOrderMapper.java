...
public interface VideoOrderMapper {
    ...
    @Insert("INSERT INTO `wechatpay`.`video_order`(`openid`, `out_trade_no`, `state`, " +
            "`create_time`, `notify_time`, `total_fee`, `nickname`," +
            " `head_img`, `video_id`, `video_title`, `video_img`, `user_id`, `ip`, `del`) " +
            "VALUES (#{openid},#{outTradeNo},#{state},#{createTime},#{notifyTime}," +
    ...
    VideoOrder findByOutTradeNo(@Param("out_trade_no") String out_trade_no);
    ...
    @Update("update video_order set state=#{state}, notify_time=#{notifyTime}, " +
            "openid=#{openid} where out_trade_no=#{outTradeNo} and state=0 and del=0")
